...
export async function generateMetadata(props: {
  ...
  if (!b) return {};
  return {
    title: `Where to start with ${b.franchise.name} | Orrery`,
...
export default async function StartPage(props: { params: Promise<{ slug: string }> }) {
  const { slug } = await props.params;
  const b = getFranchise(slug);
    ...
    >
      <main className="mx-auto max-w-2xl px-6 py-14">
        ...
        <header className="mt-4 mb-8">
          <h1 className="display text-4xl font-semibold tracking-tight">Where to start</h1>
          <p className="mt-2 max-w-lg text-sm text-[var(--ink)]/65">
            {b.works.length} works is a lot of doors. Answer two questions and
            we&apos;ll point you at the right one - every path here is curated, with
