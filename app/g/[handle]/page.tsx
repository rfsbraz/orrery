...
export default async function GroupPage({ params }: { params: Promise<{ handle: string }> }) {
  const { handle } = await params;
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-14">
      <Link href="/groups" className="text-xs text-neutral-500 hover:text-neutral-300">
        ← Book clubs
      </Link>
      ...
      <p className="mt-4 text-sm text-neutral-400">
        Reading{" "}
        {bundle ? (
          <Link href={`/f/${group.franchiseSlug}`} className="text-neutral-200 underline">
      ...
      <section className="mt-10">
        ...
        <ol className="list-decimal space-y-1 pl-6 text-sm text-neutral-300">
          {resolved.workIds.map((id) => (
            <li key={id}>
              {bundle ? (
                <Link href={`/f/${group.franchiseSlug}#w-${id.split("/").pop()}`} className="hover:underline">
                  {titles.get(id) ?? id}
                </Link>
              ) : (
                (titles.get(id) ?? id)
