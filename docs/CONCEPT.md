...
- **Phase 3 - community**: groups / book clubs; community orders (submission + moderation); curation roles + credits. Goal: the community carries the data load.
  ...
  - *Remaining in Phase 3:* **groups / book clubs** (shared order + members + shared progress board + pace + spoiler-gated discussion), and **curation as a credited role** (profile billing, editorial reputation - `is_moderator` is the seed; a fuller curator/credits model comes with it).
