...
- **Phase 2 - the completionist's home**: accounts; public profiles; solo progress; the **personal timeline overlay**; achievements engine; Goodreads/StoryGraph import; per-country store links (Works now have Editions/ISBNs). Goal: a place that's *yours to return to*.
  ...
  - *Also working:* **Goodreads/StoryGraph import** (`/import`) - upload a CSV, matched to canon server-side, previewed (matched with status+date, unmatched flagged) before a confirmed bulk upsert. And **per-country find-a-copy links** on every work: country-tuned retailer search + exact OpenLibrary work page (via OLID) and Google Books fallbacks, country remembered in localStorage so the museum pages stay static.
  - *Phase 2 is feature-complete on the local stack* (34 unit tests + browser-verified end-to-end). **Remaining before a real beta:** (1) **provision** - Supabase Cloud recommended over homeberry self-host for managing real users' auth (see §12); (2) **affiliate tags + PT specialty retailers** (Wook/Bertrand/FNAC) once referral programs are confirmed - the `storeLinks()` engine has the seam for them.
- **Phase 3 - community**: groups / book clubs; community orders (submission + moderation); curation roles + credits. Goal: the community carries the data load.
  - *Started - community reading orders working end-to-end (local Supabase):* migration `0002_community.sql` (`reading_orders` + `order_votes` + `profiles.is_moderator`, RLS: read approved-or-own-or-moderator / write own / moderator approve-reject, `is_moderator()` security-definer helper). Submit UI at `/f/[slug]/orders/new` (pick + arrange canon works, name, rationale), community orders shown on the franchise museum (client-layered so the page stays SSG) with upvote, and a moderator queue at `/moderate`. Verified: submit → pending (hidden from public) → moderator approves → public + votable, votes persist. Community orders reference the same immutable Work IDs as canon (§4a). **Round-trip to canon** (a top-voted community order promoted to a git file by a curator) is the next content-side step.
  - *Also working - groups / book clubs (local Supabase):* migration `0003_groups.sql` (`groups` + `group_members` + security-definer helpers, and a **scoped progress RLS carve-out** so group members see each other's progress without exposing private shelves to strangers - verified in SQL: a fellow member reads a private member's progress, a non-member reads zero). `/groups` browses public clubs and creates one (franchise + a canon/curated/community order + pace); `/g/[handle]` shows the chosen order, join/leave, member roster, and a **shared progress board** (each member's read-count over the order's works). A pure `resolveOrderRef` maps `canon:default | canon:<id> | community:<uuid>` to work IDs (5 tests).
  - *Also working - curation as a credited role:* a **Curator** badge + accepted-community-order count on `/me` and `/u/[handle]`, and a moderator-only `/moderate` link. Honest (derives from real data); `is_moderator` is the role seed. A fuller credits/reputation model (per-franchise curator billing, curator achievements) can grow from here.
  - *Phase 3 is feature-complete on the local stack* (39 unit tests, production build green with museum pages still SSG). **Deferred within Phase 3:** spoiler-gated per-book group discussion, and the **round-trip to canon** (a curator promoting a top-voted community order into a git file). **Same beta blocker as Phase 2:** provisioning (Supabase Cloud recommended) - everything runs on the local Supabase CLI stack today.

- **Phase 4 - the framework stack (2026-07)**: the vision features, built framework-first. A **capability system** (`lib/content/capabilities.ts`, documented in [`docs/FEATURES.md`](FEATURES.md)) makes every advanced feature per-franchise: auto-detected from the shape of the content, overridable via `features:` in franchise.yaml, statically generated only where active. Shipped as a 9-PR stack (merge in order): framework core + **spoiler engine** (§9, finally a primitive not a field) → the **River view** (§5's soul) → **order diff** → **where-to-start wizard** (content-driven `startHere`) → **connections map** (arc diagram + spoiler-gated character threads, `[[character:id]]` real) → **reading companion mode** → **year-in-reading recap** + share card → the **cross-franchise Hall** → the **editions layer** (covers + exact-edition store links + the explicit affiliate seam). Content-side: schema v2 in orrery-content (capabilities, characters, connections, editions, startHere) and the second franchise wave - Discworld, Cosmere, Wheel of Time, Agatha Christie, João Tordo - researched via the `franchise-research` skill, each bundle exercising a different slice of the framework (WoT deliberately sparse on connections; Tordo the sparse-everything stress case).
